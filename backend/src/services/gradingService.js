 ...
 * 4. ESSAY: AI-grade hoặc external API theo cấu hình câu hỏi
 */
...
const buildExternalApiHeaders = (authHeaderValue) => {
  ...
  if (separatorIndex > 0) {
    const headerName = raw.slice(0, separatorIndex).trim();
  ...
  headers.Authorization = /^Bearer\s+/i.test(raw) || /^Basic\s+/i.test(raw) ? raw : `Bearer ${raw}`;
  return headers;
};

const callEssayExternalApi = async (metadata, payload) => {
  const targetUrl = String(metadata?.externalApiUrl || '').trim();
  if (!targetUrl) {
    throw new HttpError(400, 'External API URL is required for essay grading');
  }

  const timeoutMs = Number(process.env.ESSAY_EXTERNAL_GRADER_TIMEOUT_MS || 10000);
  const response = await axios.post(targetUrl, payload, {
    timeout: Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : 10000,
    headers: buildExternalApiHeaders(metadata?.externalApiAuthHeader),
  });
...
const gradeEssay = async (studentAnswer, metadata, questionContent) => {
  try {
    ...
    const gradingMethod = String(metadata?.gradingMethod || 'ai').toLowerCase();
    ...
    if (gradingMethod === 'external_api') {
      const externalResult = await callEssayExternalApi(metadata, {
      ...
      const rawScore = externalResult.score ?? externalResult.totalScore ?? externalResult.data?.score ?? externalResult.data?.totalScore ?? null;
      const maxScore = Number(externalResult.maxScore ?? externalResult.data?.maxScore ?? 100);
      const criteria = externalResult.criteria ?? externalResult.breakdown ?? externalResult.data?.criteria ?? externalResult.data?.breakdown ?? [];
      const overallFeedback = externalResult.feedback ?? externalResult.overallFeedback ?? externalResult.data?.feedback ?? externalResult.data?.overallFeedback ?? '';
      const score = typeof rawScore === 'number'
        ? (Number.isFinite(maxScore) && maxScore > 0 && maxScore !== 100 ? Math.round((rawScore / maxScore) * 100) : rawScore)
        : 0;
