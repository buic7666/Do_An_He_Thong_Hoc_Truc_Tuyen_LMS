...
const updateCourse = async (courseId, payload, currentUser) => {
  if (!currentUser?.id) {
    throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
  }
  ...
  // Giáo viên chỉ được sửa khóa học của chính mình
  if (currentUser.role !== 'admin' && Number(course.instructorId) !== Number(currentUser.id)) {
    throw new HttpError(403, 'Bạn chỉ được sửa khóa học của chính mình', 'FORBIDDEN');
  }

  const nextData = {};

  if (Object.prototype.hasOwnProperty.call(payload, 'title')) {
    nextData.title = payload.title;
  }
  ...
  if (Object.prototype.hasOwnProperty.call(payload, 'price')) {
    nextData.price = Number(payload.price || 0);
  }

  // Chỉ admin mới được đổi giảng viên của khóa học
  if (currentUser.role === 'admin' && payload.instructorId) {
    nextData.instructorId = payload.instructorId;
  }

  /**
   * QUAN TRỌNG:
   * Nếu giáo viên sửa tên / mô tả / giá khóa học
   * thì khóa học phải quay về trạng thái chờ admin duyệt lại.
   */
  ...
  return {
    ...
    description: plainCourse.description,
    price: Number(plainCourse.price || 0),
    instructorId: plainCourse.instructorId,
    status: plainCourse.status,
    approvalStatus: plainCourse.approvalStatus,
