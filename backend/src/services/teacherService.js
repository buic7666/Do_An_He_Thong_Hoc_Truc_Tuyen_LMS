const { Course, Enrollment, Lesson, User } = require('../models');
...
const getMyCourses = async (currentUser) => {
  if (!currentUser?.id) {
    throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
  }
  ...
  return courses.map((course) => {
    const plain = course.toJSON();
    const activeStudents = (plain.enrollments || []).filter((enrollment) => enrollment.status === 'active').length;
    ...
    return {
      ...
      instructor: plain.instructor || null,
      status: plain.status,
...
const getMyCourseLessons = async (courseId, currentUser) => {
  if (!currentUser?.id) {
    throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
  }
  ...
  if (!Number.isInteger(parsedCourseId) || parsedCourseId <= 0) {
    throw new HttpError(400, 'Invalid courseId', 'INVALID_COURSE_ID');
  }
  ...
  if (currentUser.role !== 'admin' && Number(course.instructorId) !== Number(currentUser.id)) {
    throw new HttpError(403, 'Bạn chỉ được xem bài học trong khóa học của chính mình', 'FORBIDDEN');
  }
