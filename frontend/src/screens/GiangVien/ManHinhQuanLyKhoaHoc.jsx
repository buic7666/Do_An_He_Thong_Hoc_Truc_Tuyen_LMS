...
function ManHinhQuanLyKhoaHoc() {
  ...
  const handleSaveLesson = async () => {
    ...
    const videoUrl = editingLesson.videoUrl.trim();
          ...
          <div style={{ marginTop: '24px', borderTop: '1px solid #e5e7eb', paddingTop: '20px' }}>
            ...
            {selectedLessonEmbedUrl ? (
              <div style={{ marginTop: '24px' }}>
                <h4 style={{ marginBottom: '12px' }}>Preview YouTube</h4>
                <div style={{ position: 'relative', paddingTop: '56.25%', borderRadius: '16px', overflow: 'hidden', background: '#000' }}>
                  <iframe
                    key={`${selectedLessonId}-${previewNonce}-${previewStartSeconds}`}
          ...
          {editingLesson ? (
            <div className="instructor-course-builder-grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              <div>
                ...
                <div className="instructor-course-builder-form-group">
                  <label className="instructor-course-builder-form-label" htmlFor="edit-lesson-video">Link YouTube</label>
                  <input
                    className="instructor-course-builder-form-control"
                    id="edit-lesson-video"
                    value={editingLesson.videoUrl}
                    onChange={(event) => setEditingLesson((previous) => ({ ...previous, videoUrl: event.target.value }))}
                  />
                </div>
